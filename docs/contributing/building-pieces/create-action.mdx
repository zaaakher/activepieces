...
<Warning>
Using the regular `docker-compose.yaml` instead of `docker-compose-dev.yaml` will not work properly, as the local components are only visible in development mode.
</Warning>
